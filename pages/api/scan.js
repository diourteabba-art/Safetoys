...
export default async function handler(req, res) {
  ...
  // Nettoyer le code — supprimer zéros de tête, espaces, tirets
  code = code.trim().replace(/[-\s]/g, '');

  // Essayer plusieurs variantes du code
  const variants = new Set([
    code,
    code.replace(/^0+/, ''),           // sans zéros de tête
    code.padStart(13, '0'),            // padded à 13 chiffres
    code.padStart(12, '0'),            // padded à 12 chiffres
    code.length === 12 ? '0' + code : code, // UPC → EAN
  ]);
  ...
  try {
    ...
    return res.status(404).json({ error: 'Jouet non trouvé', codesTestes: [...variants] });
  } catch (e) {
